...
import {
  MATRICULA_CREATE_RESET,
  MATRICULA_DELETE_RESET,
...
function CursoActions(props) {
  ...
  const handleMatricula = () => {
    dispatch(
      createMatricula({ aluno_id: aluno.aluno_id, curso_id: curso.curso_id })
    );
  };

  const handleDesmatricula = () => {
    dispatch(
      deleteMatricula({ aluno_id: aluno.aluno_id, curso_id: curso.curso_id })
    );
  };
  ...
  return (
    <Grid container spacing={2} className={classes.container}>
      {aluno &&
      aluno.assinatura &&
      Number(aluno.assinatura.plano_id) >= Number(curso.plano_id) ? (
