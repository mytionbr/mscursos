...
import {
  ...
  AULA_DELETE_SUCCESS,
  AULA_DETAILS_FAIL,
  AULA_DETAILS_REQUEST,
  AULA_DETAILS_SUCCESS,
  AULA_FIND_FAIL,
  AULA_FIND_REQUEST,
  AULA_FIND_SUCCESS,
  AULA_FINISH_FAIL,
  AULA_FINISH_REQUEST,
  AULA_FINISH_SUCCESS,
  AULA_INFORMATIONS_FAIL,
  AULA_INFORMATIONS_REQUEST,
  AULA_INFORMATIONS_SUCCESS,
  AULA_INFO_LIST_FAIL,
  AULA_INFO_LIST_REQUEST,
  AULA_INFO_LIST_SUCCESS,
  AULA_UPDATE_FAIL,
  AULA_UPDATE_REQUEST,
  AULA_UPDATE_SUCCESS,
} from "../constants/aulaConstantes";
...
export const detailsAula = (aulaId, cursoId) => async (dispatch, getState) => {
  dispatch({
    type: AULA_DETAILS_REQUEST,
    payload: { aulaId: aulaId, cursoId: cursoId },
  });
  const {
    professorSignin: { professorInfo },
  } = getState();
...
export const findAulasInfo = (cursoSlug) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await Api.findAulasInfoByCursoAndAluno(
      cursoSlug,
      alunoInfo
    );
