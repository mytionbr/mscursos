...
import {
  ASSINATURA_CREATE_FAIL,
  ASSINATURA_CREATE_REQUEST,
  ASSINATURA_CREATE_SUCCESS,
} from "../constants/assinaturaConstantes";

export const createAssinatura =
  (plan, paymentResult) => async (dispatch, getState) => {
    dispatch({
      type: ASSINATURA_CREATE_REQUEST,
      payload: { plan, paymentResult },
    });
    try {
      const {
      ...
      const { data } = await Api.createAssinatura(
        price,
        idPlano,
        paymentResult,
        alunoInfo
